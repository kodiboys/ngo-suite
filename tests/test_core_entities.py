...
class TestDonationProperties:
    ...
    @given(amount=st.decimals(min_value=10000.01, max_value=1000000, places=2))
    def test_money_laundering_flag(self, amount):
        """>10.000€ löst Geldwäsche-Flag aus"""
        donation = Donation(
            amount=amount, donor_email_pseudonym="test@example.com", payment_intent_id="test_123"
        )
...
class DonationStateMachine(RuleBasedStateMachine):
    ...
    def create_donation(self, amount):
        ...
        self.donation = Donation(
            amount=amount, donor_email_pseudonym="test@example.com", payment_intent_id=str(uuid4())
        )
        self.amount = amount
...
async def test_event_sourcing_replay():
    ...
    async with async_session() as session:
        ...
        replayed_events = await session.execute(
            select(EventStore)
            .where(EventStore.aggregate_id == events[0].aggregate_id)
