...
class MFAManager:
    ...
    def setup_mfa(self, user_email: str) -> dict[str, str]:
        ...
        # In Production: QR Code als Base64 generieren
        return {
            "secret": secret,
...
class AuthService:
    ...
    async def login(self, request: Request, login_data: LoginRequest, ip_address: str) -> LoginResponse:
        ...
        async with self.session_factory() as session:
            ...
            if not user or not self.password_manager.verify_password(login_data.password, user.password_hash):
    ...
    async def setup_mfa(self, user_id: UUID) -> MFASetupResponse:
        ...
        async with self.session_factory() as session:
            ...
            await session.execute(
                ...
                .values(mfa_secret=mfa_setup["secret"])
            )
            await session.commit()
    ...
    async def _log_audit(self, user_id: UUID | None, action: str, entity_type: str,
                         entity_id: UUID | None, ip_address: str, reason: str = None,
...
async def get_current_active_user(
    ...
    # Hier könnten weitere Checks wie Email-Verifikation kommen
    return current_user
