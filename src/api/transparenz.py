...
def generate_transparency_hash(donor_email: str, salt: str | None = None) -> str:
    ...
    hash_obj = hmac.new(salt.encode(), donor_email.lower().encode(), hashlib.sha256)
...
async def get_transparency_data(
    ...
    stmt = select(
        ...
        Donation.created_at.between(start_date, end_date), Donation.payment_status == "succeeded"
    )
