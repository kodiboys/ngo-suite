...
class MerkleTreeService:
    ...
    async def _get_previous_day_hash(self, target_date: date) -> Optional[str]:
        ...
        stmt = select(TransparencyHash).where(TransparencyHash.date == prev_date)
        result = await self.session.execute(stmt)
    ...
    async def get_daily_root(self, year: int, month: int = None, day: int = None) -> str:
        ...
        if day:
            ...
            stmt = (
                select(TransparencyHash)
                .where(TransparencyHash.year == year, TransparencyHash.month == month)
                .order_by(TransparencyHash.date.desc())
            )
        else:
            stmt = (
                select(TransparencyHash)
                .where(TransparencyHash.year == year)
                .order_by(TransparencyHash.date.desc())
            )
    ...
    async def generate_monthly_report(self, year: int, month: int) -> bytes:
        ...
        for h in hashes:
            data.append(
                [h.date.strftime("%d.%m.%Y"), h.merkle_root[:20] + "...", str(h.record_count)]
            )
