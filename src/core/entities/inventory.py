...
class StockMovement(Base):
    ...
    need_fulfillment_id = Column(
        PGUUID(as_uuid=True), nullable=True
    )  # Referenz zur Bedarfserfüllung
    ...
    )  # donation, order, packinglist, need_fulfillment
    ...
    def __post_init__(self):
        self.new_quantity = (
            ...
            if self.movement_type == StockMovementType.INBOUND
            else self.previous_quantity - self.quantity
        )
...
class PackingListItem(Base):
    ...
    __table_args__ = (Index("idx_packinglist_item", "packing_list_id", "item_id"),)

    id = Column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    packing_list_id = Column(
        PGUUID(as_uuid=True), ForeignKey("packing_lists.id", ondelete="CASCADE"), nullable=False
    )
    item_id = Column(
        PGUUID(as_uuid=True), ForeignKey("inventory_items.id", ondelete="RESTRICT"), nullable=False
    )
