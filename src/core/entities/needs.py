...
class ProjectNeed(Base):
    ...
    inventory_item_id = Column(PGUUID(as_uuid=True), ForeignKey("inventory_items.id", ondelete="SET NULL"), nullable=True)
    ...
    def is_urgent(self) -> bool:
        ...
        return self.priority in [NeedPriority.CRITICAL, NeedPriority.HIGH] and self.remaining_quantity > 0
...
class NeedHistory(Base):
    ...
    id = Column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    need_id = Column(PGUUID(as_uuid=True), ForeignKey("project_needs.id", ondelete="CASCADE"), nullable=False)
