...
from src.core.entities.base import Donation, Project, SKR42Account, AuditLog
from src.core.events.event_bus import EventBus, Event
from src.core.entities.transparency import Transaction
...
class AccountingService:
    ...
    async def book_donation(self, donation_id: UUID, user_id: UUID) -> dict[str, Any]:
        ...
        async with self.session_factory() as session:
            ...
            return {"booking_id": str(transaction.id), "booking": booking, "status": "completed"}

    async def _save_booking(
        self, session: AsyncSession, booking: dict[str, Any], user_id: UUID, donation_id: UUID
    ) -> Transaction:
        """Speichert Buchung in der Datenbank"""

        # Hole Konten-IDs
        debit_stmt = select(SKR42Account).where(
            SKR42Account.account_number == booking["debit_account"]
        )
        debit_result = await session.execute(debit_stmt)
        debit_account = debit_result.scalar_one()

        credit_stmt = select(SKR42Account).where(
            SKR42Account.account_number == booking["credit_account"]
        )
    ...
    async def export_datev_csv(
        ...
        async with self.session_factory() as session:
            ...
            for transaction in transactions:
                # Hole Kontonummern
                debit_stmt = select(SKR42Account).where(
                    SKR42Account.id == transaction.debit_account_id
                )
                debit_result = await session.execute(debit_stmt)
                debit_account = debit_result.scalar_one()

                credit_stmt = select(SKR42Account).where(
                    SKR42Account.id == transaction.credit_account_id
                )
                credit_result = await session.execute(credit_stmt)
                credit_account = credit_result.scalar_one()
    ...
    async def export_datev_fuxt(self, start_date: datetime, end_date: datetime) -> bytes:
        ...
        async with self.session_factory() as session:
            ...
            for transaction in transactions:
                ...
                xml_lines.append(
                    f'            <datev:date>{transaction.booking_date.strftime("%Y-%m-%d")}</datev:date>'
                )
                xml_lines.append(
                    f"            <datev:debit_account>{debit_account.account_number}</datev:debit_account>"
                )
                xml_lines.append(
                    f"            <datev:credit_account>{credit_account.account_number}</datev:credit_account>"
                )
                xml_lines.append(
                    f"            <datev:amount>{transaction.amount:.2f}</datev:amount>"
                )
                xml_lines.append(
                    f"            <datev:description>{self._escape_xml(transaction.description)}</datev:description>"
                )
                if transaction.cost_center:
                    xml_lines.append(
                        f"            <datev:cost_center>{transaction.cost_center}</datev:cost_center>"
                    )
                xml_lines.append("        </datev:transaction>")
    ...
    async def get_account_balance(
        self, account_number: str, year: int, project_id: UUID | None = None
    ) -> Decimal:
    ...
    async def get_profit_and_loss(
        self, year: int, project_id: UUID | None = None
    ) -> dict[str, Any]:
        """
    ...
    async def get_balance_sheet(self, year: int, project_id: UUID | None = None) -> dict[str, Any]:
        """
