...
from src.core.entities.base import Project, Donation, AuditLog, User
from src.core.entities.inventory import (
    InventoryItem, StockMovement, PackingList, PackingListItem,
    InventoryItemCreate, StockMovementCreate, PackingListCreate,
    StockMovementType, StockStatus, ItemCategory, ItemCondition
)
...
class InventoryService:
    ...
    async def create_item(self, item_data: InventoryItemCreate, user_id: UUID) -> InventoryItem:
        ...
        async with self.session_factory() as session:
            ...
            if item_data.need_id:
                ...
                if not need:
                    raise HTTPException(status_code=404, detail=f"Need {item_data.need_id} not found")
    ...
    async def update_stock(self, movement: StockMovementCreate, user_id: UUID, ip_address: str) -> StockMovement:
        ...
        async with self.session_factory() as session:
            ...
            if item.unit_price and movement.movement_type in [StockMovementType.INBOUND, StockMovementType.OUTBOUND, StockMovementType.NEED_FULFILLMENT]:
            ...
            if movement.movement_type == StockMovementType.NEED_FULFILLMENT and movement.need_id:
                await self._update_need_on_fulfillment(movement.need_id, movement.quantity, user_id, session)
    ...
    async def get_low_stock_for_needs(self, project_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
    ...
    async def get_transparency_inventory(self, project_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
        """Holt Lagerbestände für Transparenzseite (öffentlich)"""
        async with self.session_factory() as session:
            stmt = select(InventoryItem).where(
                InventoryItem.show_on_transparency == True,
                InventoryItem.is_active == True
            )
    ...
    async def confirm_packing_list(self, packing_list_id: UUID, user_id: UUID) -> PackingList:
        ...
        async with self.session_factory() as session:
            ...
            for item in packing_list.items:
                movement_type = StockMovementType.NEED_FULFILLMENT if item.need_id else StockMovementType.OUTBOUND
    ...
    async def mark_as_delivered(self, packing_list_id: UUID, signature_data: Optional[str] = None) -> PackingList:
        """
    ...
    async def _create_skr42_booking_for_movement(self, movement: StockMovement, item: InventoryItem, session):
        ...
        elif movement.movement_type in [StockMovementType.OUTBOUND, StockMovementType.NEED_FULFILLMENT]:
    ...
    async def get_need_fulfillment_history(self, need_id: UUID, limit: int = 50) -> List[Dict[str, Any]]:
