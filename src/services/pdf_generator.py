...
class DonationReceiptGenerator:
    ...
    async def generate_donation_receipt(self, donation_id: UUID, include_personal_data: bool = True) -> bytes:
        """
        ...
        async with self.session_factory() as session:
            ...
            if include_personal_data:
                ...
                donor_info = [
                    ...
                    ["PLZ, Ort:", "---"],
                    ["E-Mail:", donation.donor_email_pseudonym[:30] + "..." if donation.donor_email_pseudonym else "---"]
                ]
...
class SKR42BalanceSheetGenerator:
    ...
    async def generate_balance_sheet(
        ...
        async with self.session_factory() as session:
            ...
            active_data = [
                ["Konto", "Bezeichnung", f"Saldo {year}", f"Saldo {year-1}" if previous_year_data else "Vorjahr"]
            ]
            ...
            for account in balance_data.get('active', []):
                active_data.append([
                    ...
                    f"{previous_year_data['active'][idx]['balance']:,.2f} €" if previous_year_data and idx < len(previous_year_data['active']) else "---"
                ])

            # Summe Aktiva
            total_active = sum(a['balance'] for a in balance_data.get('active', []))
            active_data.append([
                "", "SUMME AKTIVA", f"{total_active:,.2f} €",
                f"{sum(a['balance'] for a in previous_year_data.get('active', [])):,.2f} €" if previous_year_data else "---"
            ])
            ...
            for account in balance_data.get('passive', []):
                passive_data.append([
                    ...
                    account['account_name'],
                    f"{account['balance']:,.2f} €",
                    f"{previous_year_data['passive'][idx]['balance']:,.2f} €" if previous_year_data and idx < len(previous_year_data['passive']) else "---"
                ])

            # Summe Passiva
            total_passive = sum(a['balance'] for a in balance_data.get('passive', []))
            passive_data.append([
                "", "SUMME PASSIVA", f"{total_passive:,.2f} €",
                f"{sum(a['balance'] for a in previous_year_data.get('passive', [])):,.2f} €" if previous_year_data else "---"
            ])
            ...
            kpi_data = [
                ...
                ["Eigenkapitalquote", f"{self._calculate_equity_ratio(balance_data):.1f}%",
                 f"{self._calculate_equity_ratio(previous_year_data):.1f}%" if previous_year_data else "---",
                 self._get_trend_symbol(self._calculate_equity_ratio(balance_data), self._calculate_equity_ratio(previous_year_data))],
                ["Liquidität 3. Grades", f"{self._calculate_liquidity(balance_data):.1f}%",
                 f"{self._calculate_liquidity(previous_year_data):.1f}%" if previous_year_data else "---",
                 self._get_trend_symbol(self._calculate_liquidity(balance_data), self._calculate_liquidity(previous_year_data))],
                ["Spendenaufkommen", f"{balance_data.get('donations_total', 0):,.2f} €",
                 f"{previous_year_data.get('donations_total', 0):,.2f} €" if previous_year_data else "---",
                 self._get_trend_symbol(balance_data.get('donations_total', 0), previous_year_data.get('donations_total', 0) if previous_year_data else 0)],
                ["Projektkostenquote", f"{balance_data.get('project_cost_ratio', 0):.1f}%",
                 f"{previous_year_data.get('project_cost_ratio', 0):.1f}%" if previous_year_data else "---",
                 self._get_trend_symbol(balance_data.get('project_cost_ratio', 0), previous_year_data.get('project_cost_ratio', 0) if previous_year_data else 0)]
            ]
    ...
    async def _get_balance_data(self, session: AsyncSession, project_id: UUID | None, year: int) -> dict[str, Any]:
        ...
        # Berechne Kennzahlen
        project_cost_ratio = (donations_total / (sum(a['balance'] for a in active_accounts) + 1)) * 100
...
class ProjectReportGenerator:
    ...
    async def generate_project_report(self, project_id: UUID, include_donors: bool = False) -> bytes:
        ...
        async with self.session_factory() as session:
            ...
            doc = SimpleDocTemplate(
                buffer,
                pagesize=A4,
                title=f"Projektbericht_{project.name}"
            )
            ...
            # Projekt-Overview
            overview_data = [
                ["Projektzeitraum:", f"{project.start_date.strftime('%d.%m.%Y')} - {project.end_date.strftime('%d.%m.%Y') if project.end_date else 'laufend'}"],
                ...
                ["Fortschritt:", f"{(project.budget_used / project.budget_total * 100) if project.budget_total > 0 else 0:.1f}%"]
            ]
            ...
            # Fortschrittsbalken
            progress = (project.budget_used / project.budget_total * 100) if project.budget_total > 0 else 0
    ...
    async def _get_project_donations(self, session: AsyncSession, project_id: UUID) -> list[Donation]:
